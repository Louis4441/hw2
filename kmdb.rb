...
c_bale = Person.where({ name: "Christian Bale" })[0]
m_caine = Person.where({ name: "Michael Caine" })[0]
l_neeson = Person.where({ name: "Liam Neeson" })[0]
k_holmes = Person.where({ name: "Katie Holmes" })[0]
g_oldman = Person.where({ name: "Gary Oldman" })[0]
h_ledger = Person.where({ name: "Heath Ledger" })[0]
a_eckhart = Person.where({ name: "Aaron Eckhart" })[0]
m_gyllenhaal = Person.where({ name: "Maggie Gyllenhaal" })[0]
t_hardy = Person.where({ name: "Tom Hardy" })[0]
j_g_levitt = Person.where({ name: "Joseph Gordon-Levitt" })[0] 
a_hathaway = Person.where({ name: "Anne Hathaway"  })[0]
